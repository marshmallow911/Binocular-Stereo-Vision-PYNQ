...
#ifndef _disparityMap_leftKfY_H_
#define _disparityMap_leftKfY_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "disparityMap_leftKfY_memcore.h"

SC_MODULE(disparityMap_leftKfY) {
...
protected:
    ...
    // buffer instances
    disparityMap_leftKfY_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(disparityMap_leftKfY) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "disparityMap_leftKfY_memcore_U_";
            name += szTmp;

            buffer[i] = new disparityMap_leftKfY_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "disparityMap_leftKfY.trace");
            // ports
